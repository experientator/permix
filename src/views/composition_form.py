...
from src.utils.database_utils import (get_templates_list, get_template_id, get_template_sites,
...
class CompositionView(tk.Toplevel):
    ...
    def create_template_frame(self):
        ...
        self.mi_label6 = tk.Label(self.main_info_frame,
                                  ...
                                  **AppStyles.label_style())
        self.mi_label6.grid(row=4, column=1, columnspan=2, sticky='ew', padx=5, pady=2)
        self.entry_notes = tk.Entry(self.main_info_frame,
                                    **AppStyles.entry_style())
        self.entry_notes.grid(row=5, column=1, columnspan=2, sticky='ew', padx=5, pady=2)

        self.antisolv_check = tk.IntVar(self)
        self.antisolvents_cb = tk.Checkbutton(self.main_info_frame,
                                              text=localization_manager.tr("ucv_cb1"),
                                              variable=self.antisolv_check,
                                              **AppStyles.checkbutton_style())
        self.antisolvents_cb.grid(row=5, column=0, columnspan=1, sticky='ew', padx=5, pady=2)
    ...
    def upload_data(self, upd = False, id_info = None):
        ...
        self.controller.handle_main_submit(main_info,  solution_info, structure_data,
                           solvents, properties, k_factors, upd, id_info)
    ...
    def update_composition(self, id_info=None, not_fav = None):
        ...
        extra_param = get_extra_parameters('data.db', id_info, not_fav)
        self.entry_template.set(template_name)

        self.entry_name.insert(0, extra_param["name"] or "")
        self.entry_notes.insert(0, extra_param["notes"] or "")
        if not_fav:
            self.entry_data_type.set(extra_param["data_type"])
            self.entry_device_type.set(extra_param["device_type"])
            self.entry_doi.insert(0, extra_param["doi"] or "")

            self.create_properties_frame(extra_param["device_type"])
            properties = get_device_properties(id_info, extra_param["device_type"])
            for i, entry in enumerate(self.property_entries):
                entry.insert(0, properties[i] or "")
        else:
            self.entry_data_type.insert(0, "")
            self.entry_data_type.config(state="disabled")
            self.entry_device_type.insert(0, "")
            self.entry_device_type.config(state="disabled")
            self.entry_doi.insert(0, "")
            self.entry_doi.config(state="disabled")
        self.create_sites()
        if antisol_num != 0:
        ...
        if not_fav:
            self.entry_stab_notes.insert(0, extra_param["stab_notes"] or "")
            self.entry_method_desc.insert(0, extra_param["method_desc"] or "")

        else:
            self.entry_stab_notes.config(state="disabled")
            self.entry_method_desc.config(state="disabled")
        ...
        self.create_submit_button()
        self.subm_button.config(text=localization_manager.tr("cfv_upd_comp"))

        if not_fav:
            self.subm_button.config(command=lambda: self.upload_data(upd=True, id_info=id_info))
        else:
            self.subm_button.config(command=lambda: self.upload_favorite_data(upd=True, id_info=id_info))

    def upload_favorite_data(self, upd = False, id_info = None):
        solvents_data, solution_info = self.get_solution_data()
        k_factors_data = self.get_k_factors_data()
        cations_data, anions_data = self.get_fav_structure_data()
        notes_fav = self.entry_notes.get()
        name_fav = self.entry_name.get()
        name = self.entry_template.get()
        id_phase = get_template_id(name)
        cont_ucv = self.get_ucv_controller()
        cont_ucv.handle_main_submit(name_fav, notes_fav, solvents_data, cations_data,
                                    anions_data, k_factors_data, solution_info, id_phase, upd, id_info)

    def get_fav_structure_data(self):
        used_symbols = set()
        cations = []
        anions = []
        for idx, site_type in enumerate(self.structure_types):
    ...
    def get_ucv_controller(self):
        if self.ucv_cont is None:
            self.ucv_cont = UserConfigController(self)
        return self.ucv_cont